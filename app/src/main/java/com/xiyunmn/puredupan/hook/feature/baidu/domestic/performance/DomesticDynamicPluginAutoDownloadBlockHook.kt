 ...
 */
object DomesticDynamicPluginAutoDownloadBlockHook {
    ...
    internal fun hook(cl: ClassLoader) {
        if (!isEnabled()) {
            XposedCompat.log("[DomesticDynamicPluginAutoDownloadBlockHook] skipped: config disabled")
            return
        }
        ...
        try {
            val pluginClass = XposedCompat.findClassOrNull(
                BaiduDomesticHookPoints.DYNAMIC_PLUGIN_MODEL,
                cl,
            ) ?: run {
                XposedCompat.log("[DomesticDynamicPluginAutoDownloadBlockHook] DynamicPlugin class NOT FOUND")
                hookState.reset()
                return
            }
            ...
            installedCount += DomesticDynamicPluginAutoDecisionDexKitResolver.hookResolvedDecisionFactories(
                ...
                logTag = "DomesticDynamicPluginAutoDownloadBlockHook",
                isEnabled = ::isEnabled,
            )
            if (installedCount == 0) {
                installedCount += hookDecisionClasses(
                    ...
                    classNames = BaiduDomesticHookPoints.DYNAMIC_PLUGIN_AUTO_DOWNLOAD_DOWNLOADER_CLASSES,
                    methodName = BaiduDomesticHookPoints.DYNAMIC_PLUGIN_IS_AUTO_DOWNLOAD_METHOD,
                    decisionName = "autoDownload",
                )
                installedCount += hookDecisionClasses(
                    cl = cl,
                    pluginClass = pluginClass,
                    classNames = BaiduDomesticHookPoints.DYNAMIC_PLUGIN_AUTO_INSTALL_EXECUTOR_CLASSES,
                    methodName = BaiduDomesticHookPoints.DYNAMIC_PLUGIN_IS_AUTO_INSTALL_METHOD,
                    decisionName = "autoInstall",
                )
            }

            if (installedCount == 0) {
                XposedCompat.log("[DomesticDynamicPluginAutoDownloadBlockHook] no hooks installed")
                hookState.reset()
                return
            }

            XposedCompat.log("[DomesticDynamicPluginAutoDownloadBlockHook] hooks INSTALLED: count=$installedCount")
        } catch (e: Exception) {
            hookState.reset()
            XposedCompat.log("[DomesticDynamicPluginAutoDownloadBlockHook] FAILED: ${e.message}")
            XposedCompat.log(e)
        }
    }
    ...
    private fun hookDecisionClasses(
        ...
        for (className in classNames) {
            ...
            if (clazz == null) {
                XposedCompat.log("[DomesticDynamicPluginAutoDownloadBlockHook] $className NOT FOUND")
                continue
            }
            val method = XposedCompat.findMethodOrNull(clazz, methodName, pluginClass)
            if (method == null) {
                XposedCompat.log("[DomesticDynamicPluginAutoDownloadBlockHook] $className.$methodName NOT FOUND")
                continue
            }
            mod.hook(method).intercept { chain ->
                ...
                if (
                    ...
                    XposedCompat.logD(
                        "[DomesticDynamicPluginAutoDownloadBlockHook] $decisionName blocked: " +
