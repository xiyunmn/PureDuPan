...
object UiText {
    object Settings {
        ...
        const val STORAGE_PATH_MANAGEMENT_LABEL = "存储路径管理"
        const val STORAGE_PATH_MANAGEMENT_DESC = "选择 SAF 公共目录，重定向下载并保护公共根目录；默认关闭"
        const val STORAGE_DOWNLOAD_REDIRECT_LABEL = "下载重定向"
        const val STORAGE_DOWNLOAD_REDIRECT_DESC = "普通下载、分享、URL/H5、BT/P2P 使用所选公共目录；失效时阻止任务，不回退旧目录"
        const val STORAGE_ROOT_GUARD_LABEL = "公共根目录保护"
        const val STORAGE_ROOT_GUARD_DESC = "阻止百度宿主在 /storage/emulated/0 创建未知首级目录，不拦截普通文件读写"
        ...
        const val STORAGE_WECHAT_BACKUP_DESC = "将 WeChatCloudBackup 备份/恢复目录改到所选目录下；仅三星版显示"
