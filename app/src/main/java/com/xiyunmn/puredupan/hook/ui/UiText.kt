...
object UiText {
    object Settings {
        ...
        const val REMOVE_HOT_START_SPLASH_DESC = "拦截切回前台时拉起的切屏加载/热启动启动图，不影响冷启动开屏处理。启用 DexKit 解析后生效"
        ...
        const val HIDE_MEMBER_CARD_FIRST_BENEFIT_DESC = "仅国际版。隐藏会员卡片第一张等级福利卡片（cl_first_card）"
        const val HIDE_MEMBER_CARD_SECOND_BENEFIT_LABEL = "等级福利卡片 2"
        const val HIDE_MEMBER_CARD_SECOND_BENEFIT_DESC = "仅国际版。隐藏会员卡片第二张等级福利卡片（cl_second_card）"
        const val HIDE_MEMBER_CARD_THIRD_BENEFIT_LABEL = "专属权益"
        const val HIDE_MEMBER_CARD_THIRD_BENEFIT_DESC = "仅国际版。隐藏会员卡片第三张专属权益卡片（cl_third_card）；当等级福利与专属权益同时隐藏时，会一并隐藏分隔线（view_line）"
        ...
        const val HIDE_INTL_MEMBER_CARD_SVIP_LEVEL_DESC = "仅国际版。隐藏会员卡片顶部的 SVIP 等级徽标（iv_vip_image）"
        const val HIDE_INTL_MEMBER_CARD_UPGRADE_BUTTON_LABEL = "升级按钮"
        const val HIDE_INTL_MEMBER_CARD_UPGRADE_BUTTON_DESC = "仅国际版。隐藏会员卡片右上角的“去升级”按钮（tv_enter）"
        ...
        const val PERFORMANCE_GROUP_INTL_STARTUP_DELAY = "国际版启动期延后"
        ...
        const val BLOCK_INTL_OFFLINE_PACKAGE_INIT_DESC = "仅国际版。启动期阻止 H5 离线包配置监听和自动下载队列初始化；进入 H5/离线包页面前按需恢复"
        ...
        const val BLOCK_INTL_STORY_DOUYIN_INIT_DESC = "仅国际版。启用 DexKit 解析后生效。启动期阻止 Story 视频预加载、Story UI 服务与 DouYin SDK 初始化；进入 Story/短视频或 DouYin 分享/授权入口时按需恢复"
        ...
        const val BLOCK_INTL_AIGC_WIDGET_BACKGROUND_DESC = "仅国际版。阻止 AIGC 小组件后台刷新和资源解压，不拦截用户主动打开 AI 页面"
        const val BLOCK_INTL_ALBUM_AI_INIT_LABEL = "阻止相册 AI 初始化"
        const val BLOCK_INTL_ALBUM_AI_INIT_DESC = "仅国际版。启动期阻止相册 AI/NLP/AILab 初始化；进入 AI 面板、AI 搜图、NLP 预览或 AILab 入口时按需恢复"
