...
internal object DomesticLaunchHandoffOptimizeHook {
    private const val NAVIGATE_CLASS_NAME = BaiduDomesticHookPoints.NAVIGATE_ACTIVITY
    ...
    internal fun hook(cl: ClassLoader) {
        if (!HookSettings.isSplashInterstitialBlockEnabled) {
            XposedCompat.log("[DomesticLaunchHandoffOptimizeHook] skipped: config disabled")
            return
        }
        ...
        try {
            ...
            if (navigateGetLayoutIdMethod != null) {
                ...
                XposedCompat.log("[DomesticLaunchHandoffOptimizeHook] Navigate.getLayoutId NOT FOUND; layout suppression skipped")
            }
            mod.hook(setSystemUiVisibilityMethod).intercept { chain ->
                ...
                if (sanitizedVisibility != visibility) {
                    XposedCompat.logD(
                        "[DomesticLaunchHandoffOptimizeHook] sanitized shell View.setSystemUiVisibility: " +
            ...
            mod.hook(windowAddFlagsMethod).intercept { chain ->
                ...
                if (sanitizedFlags != flags) {
                    XposedCompat.logD(
                        "[DomesticLaunchHandoffOptimizeHook] sanitized shell Window.addFlags: " +
            ...
            mod.hook(startActivityMethod).intercept { chain ->
                ...
                if (shouldAccelerate && activity != null) {
                    ...
                    XposedCompat.logD(
                        "[DomesticLaunchHandoffOptimizeHook] captured Navigate handoff, waiting for MainActivity focus",
                    )
                }
                result
            }
            ...
            XposedCompat.log(
                "[DomesticLaunchHandoffOptimizeHook] hooks INSTALLED: " +
            ...
            hookState.reset()
            XposedCompat.log("[DomesticLaunchHandoffOptimizeHook] install FAILED: ${t.message}")
            XposedCompat.log(t)
        }
    }
    ...
    private fun logNavigateLayoutSuppressed() {
        ...
        XposedCompat.logD("[DomesticLaunchHandoffOptimizeHook] Navigate welcome layout suppressed")
    }
    ...
    private fun hookOptionalInsetsHideMethods(cl: ClassLoader) {
        ...
        runCatching {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                WindowInsetsController::class.java.getDeclaredMethod(
                    ...
                    if (Modifier.isAbstract(method.modifiers)) {
                        XposedCompat.logD("[DomesticLaunchHandoffOptimizeHook] platform insets hide hook skipped: abstract method")
                        return@runCatching
                    }
                    mod.hook(method).intercept { chain ->
                        ...
                        if (isShellInitViewActive() && shouldBlockShellInsetsHide(types)) {
                            XposedCompat.logD(
                                "[DomesticLaunchHandoffOptimizeHook] skipped shell WindowInsetsController.hide: " +
            ...
            XposedCompat.logD("[DomesticLaunchHandoffOptimizeHook] platform insets hide hook skipped: ${it.message}")
        }
        ...
        runCatching {
            ...
            mod.hook(hideMethod).intercept { chain ->
                ...
                if (isShellInitViewActive() && shouldBlockShellInsetsHide(types)) {
                    XposedCompat.logD(
                        "[DomesticLaunchHandoffOptimizeHook] skipped shell WindowInsetsControllerCompat.hide: " +
                            "0x${types.toString(16)}",
                    )
                    null
                } else {
            ...
            XposedCompat.logD("[DomesticLaunchHandoffOptimizeHook] compat insets hide hook skipped: ${it.message}")
        }
    }
    ...
    private fun finishPendingNavigate() {
        ...
        runCatching {
            ...
            activity.finish()
            suppressTransition(activity)
            XposedCompat.logD("[DomesticLaunchHandoffOptimizeHook] Navigate finished after MainActivity gained focus")
        }.onFailure {
            XposedCompat.logD("[DomesticLaunchHandoffOptimizeHook] delayed finish failed: ${it.message}")
        }
    }
    ...
    private fun applyStartupStatusBar(activity: Activity, reason: String) {
        ...
        runCatching {
            ...
            XposedCompat.logD(
                "[DomesticLaunchHandoffOptimizeHook] startup status bar stabilized: " +
            ...
            XposedCompat.logD("[DomesticLaunchHandoffOptimizeHook] status bar stabilize failed: ${it.message}")
        }
    }
    ...
    private fun clearShellWindowBackground(activity: Activity, reason: String) {
        ...
        runCatching {
            ...
            if (!shellWindowBackgroundClearLogged) {
                ...
                XposedCompat.logD(
                    "[DomesticLaunchHandoffOptimizeHook] shell window background cleared: $reason",
                )
            }
        }.onFailure {
            XposedCompat.logD("[DomesticLaunchHandoffOptimizeHook] shell background clear failed: ${it.message}")
        }
    }
    ...
    private fun suppressTransition(activity: Activity) {
        runCatching {
            ...
            XposedCompat.logD("[DomesticLaunchHandoffOptimizeHook] overridePendingTransition failed: ${it.message}")
        }
    }
