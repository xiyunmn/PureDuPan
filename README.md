...
- 首页：隐藏搜索推广、AIGC 图标、工具栏、轮播图、推荐提示、回忆/转存/最近等卡片；可设置「最近」显示数量及「转存/订阅」纵向列表和项目数量。
- 搜索页：隐藏 AI 入口、推荐提示词、历史记录、智能推荐、SVIP 横幅等。
- 文件页：隐藏底部数据安全提示和转存成功弹窗中的 SVIP 优惠广告卡片。
- 下载页：隐藏游戏推荐浮窗动画、推广广告、底部会员推广区域。
- 共享页：移除广告悬浮窗和相册备份提示栏。
- 我的页：隐藏游戏中心、横幅广告、服务入口、签到红点、AI 点数资产、任务中心气泡和部分入口文案；可自动跟随会员卡高度或手动调整下方内容位置。
