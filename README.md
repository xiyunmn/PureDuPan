...
国际版按 `com.baidu.drive.app` 单独隔离适配。当前已适配启动/热启动广告、下载 WiFi 确认、夜间模式支持、会员卡片、底栏和部分性能优化。由于类名、方法名混淆更严重，涉及混淆入口的能力会依赖稳定 Activity、Rubik 生成类、语义签名或 DexKit 解析。
...
- 国际版“夜间模式跟随系统”依赖 DexKit 解析，需先启用“夜间模式支持”和 DexKit 实验解析。
- 同步宿主设置页夜间模式开关状态。
- 处理底栏「我的」头像在换肤后的刷新。
...
当前主要按 `百度网盘 13.26.8`、`百度网盘国际版 13.11.2`、`百度网盘三星版 13.26.8` 验证。由于百度网盘内部类名、方法名和页面结构可能随版本变化，部分功能在其它版本上可能失效或需要重新适配。
