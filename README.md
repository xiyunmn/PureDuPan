...
- 百度网盘：`13.25.2` ~ `13.33.7`
- 百度网盘国际版：`13.11.13`（新版首页）
- 百度网盘三星版：`13.25.2` ~ `13.33.2`

国际版已按新版 Home25 界面适配，停止维护旧国际版兼容代码，不设置宿主版本硬限制。
...
- 在弹窗屏蔽中移除首页、共享页的广告悬浮窗；可用子项按宿主显示。
- 移除下载弹窗，未连接 Wi-Fi 时可继续创建下载任务。

### 页面定制

- 首页：隐藏工具栏、搜索框提示词，以及推荐/回忆/转存/最近等板块；国内版、三星版另支持隐藏顶部推广和搜索框 AIGC 图标。可设置「最近」显示数量及「转存/订阅」纵向列表和项目数量。
- 搜索页：隐藏 AI 入口、推荐提示词、历史记录、智能推荐、SVIP 横幅等。
- 文件页：隐藏相册备份提示栏、底部数据安全提示和转存成功弹窗中的 SVIP 优惠广告卡片；隐藏安全提示时保留原有底部留白。
- 下载页：隐藏游戏推荐浮窗动画、推广广告、底部会员推广区域。
- 我的页：隐藏游戏中心、横幅广告、签到红点、AI 点数资产、任务中心气泡和部分入口文案；国内版、三星版另支持隐藏服务入口。可自动跟随会员卡高度或手动调整下方内容位置。
...
- 隐藏指定底栏 Tab。
- 国内版、三星版可隐藏 AIGC、会员入口；国际版提供首页、文件、共享、我的四项。
