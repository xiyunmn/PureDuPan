...
当前主要按 `百度网盘 13.25.2 ~ 13.28.4`、`百度网盘国际版 13.11.7 ~ 13.11.8`、`百度网盘三星版 13.25.2 ~ 13.28.4` 适配。由于百度网盘内部类名、方法名和页面结构可能随版本变化，部分功能在其它版本上可能失效或需要重新适配。

国内版和三星版存在混淆发行版与未混淆/低混淆发行版。当前反编译样本中 `13.27.8` 表现为混淆分支，`13.25.2`、`13.28.4` 等表现为未混淆或低混淆分支；这只是样本记录，维护时不得按版本号硬编码判断分支，具体以实际宿主符号、DexKit 解析和稳定类名 fallback 为准。
...
本项目基于 [ForbidAd4TieBa](https://github.com/aikavvak12una/ForbidAd4TieBa) 的模块结构思路重构开发，并使用 [DexKit](https://github.com/LuckyPray/DexKit) 辅助混淆目标解析。
